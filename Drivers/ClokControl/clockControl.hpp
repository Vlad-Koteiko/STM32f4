...
namespace drivers ::clock
{
    ...
    constexpr auto countFreqHCLK = [](const Frequency f) constexpr noexcept -> std::uint32_t {
        return f;
    };
    constexpr auto countSystemCoreClock =
        [](const Frequency f) constexpr noexcept -> std::uint32_t { return f; };
    ...
    class ClockControl
    {
        ...
        constexpr std::uint8_t getPeripheralsNumber(const std::uint8_t,
    ...
    public:
        ...
        constexpr ClockControl(const Frequency f) :
            systemCoreClock(countSystemCoreClock(f)),
         ...
         */
        void PLL_Config_Sys(std::uint16_t setPLLN,
