...
namespace drivers::clock
{
   ...
   void ClockControl::SetCalibTrimming(std::uint32_t value) const noexcept
    {
        ReadWriteRegister::modifyResetRegister(RegisterRCC::CR,0xF8);              // clear internal high-speed clock trimming
    ...
    {
        ReadWriteRegister::modifyResetRegister(RegisterRCC::CFGR,(0x7 << 10));      // APB prescaler clear
    ...
    {
        ReadWriteRegister::modifyResetRegister(RegisterRCC::CFGR,(0x7 << 13));      // APB prescaler clear
    ...
    {
        ReadWriteRegister::write_register(RegisterSysTick::LOAD,static_cast<std::uint32_t>((HCLKFrequency / ticks) - 1)); // set reload register
        ReadWriteRegister::write_register(RegisterSysTick::VAL,0);                                                        // load the SysTick Counter Value
        ReadWriteRegister::write_register(RegisterSysTick::CTRL, static_cast<std::uint32_t>(5));                          // Enable the Systick Timer
    }
...
//            disableClock = ADC1;
    ...
    void ClockControl::PLL_Config_Sys(std::uint16_t  setPLLN, std::uint16_t setPLLM, std::uint16_t setPLLP, std::uint16_t setPLLQ) const noexcept {
        ...
        ReadWriteRegister::modifySetRegister(PLLCFGR, static_cast<std::uint32_t>((1 << constants::PLLSRC) | setPLLM | (setPLLN << constants::PLLN) | (setPLLQ << constants::PLLQ) | (setPLLP << constants::PLLP)));
