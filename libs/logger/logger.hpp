...
struct Writer
{
    ...
    static void Write(const std::uint8_t* data, std::size_t size)
    {
        uart_p_2->TransmitString((void *)data, size);
    }
};
...
using Logger = LoggerImplementation<Writer>;


#define LOG(FORMAT, ...) \
{ \
  static_assert(PP_PERFORM_LOG_FORMAT_CHECK(FORMAT, ##__VA_ARGS__), ""); \
  Logger::GetInstance().Log(FORMAT##_hash, ##__VA_ARGS__); \
}
       ...
       uartOut << "hello\n\r";
       uartOut << std::hex << std::showbase << 16 << " " << x << std::endl;
...
struct get_offset<0u, std::tuple<A0, As...>, Off> : public std::integral_constant<std::size_t, Off>
{};
...
public:
    ...
    constexpr void Log_impl(std::uint32_t                  hash,
                            const std::tuple<Args...>&     args,
                            std::integer_sequence<T, I...> int_seq)
    {
        std::array<std::uint8_t, 4 + Size<Args...>> buffer;
        ((new(buffer.data()) std::uint32_t(hash)),
         ...,
         new(buffer.data() + get_offset<I, std::tuple<Holder<Args>...>>::value)
             Holder<Args>(std::get<I>(args)));
