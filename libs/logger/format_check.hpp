...
{
    return fmt[n] == c ? true : checkValidFormats(fmt, n + 1, c);
}
...
#define SUPPORTED_LL_TYPE(T, C)                                                                  \
    ...
    {                                                                                            \
        ...
        static constexpr bool check(const char (&fmt)[N], std::size_t n)                         \
        {                                                                                        \
            return n < N && n - 2 >= 0 && fmt[n] == C && fmt[n - 1] == 'l' && fmt[n - 2] == 'l'; \
        }                                                                                        \
    }
...
constexpr bool isDoubleLengthSpecifier(const char (&fmt)[N], std::size_t n)
{
    // hh | ll
    return n + 2 < N &&
           ((fmt[n] == 'h' && fmt[n + 1] == 'h') || (fmt[n] == 'l' && fmt[n + 1] == 'l'));
}
...
{
    // h | l | j | z | t | L
    return n + 1 < N && (fmt[n] == 'h' || fmt[n] == 'l' || fmt[n] == 'j' || fmt[n] == 'z' ||
                         fmt[n] == 't' || fmt[n] == 'L');
}
...
{
    return isDoubleLengthSpecifier(fmt, n) ? n + 2 : isSingleLengthSpecifier(fmt, n) ? n + 1 : n;
}
...
{
    return fmt[n] >= '0' && fmt[n] <= '9' ? nextNonLiteralPrecision(fmt, n + 1) : n;
}
...
{
    return fmt[n] >= '0' && fmt[n] <= '9' ? nextNonLiteralWidth(fmt, n + 1) : n;
}
...
struct Width<T, Ts...>
{
    ...
    static constexpr bool check(const char (&fmt)[N], std::size_t n)
    {
        ...
        return fmt[n] == '*' ? std::is_integral<T>::value && Precision<Ts...>::check(fmt, n + 1)
...
{
    return n + 1 < N &&
           (fmt[n] == '-' || fmt[n] == '+' || fmt[n] == ' ' || fmt[n] == '#' || fmt[n] == '0');
}

template<std::size_t N>
constexpr size_t nextNonFlag(const char (&fmt)[N], std::size_t n)
{
    return isFlag(fmt, n) ? nextNonFlag(fmt, n + 1) : n;
}
...
struct Format<T, Ts...>
{
    ...
    static constexpr bool check(const char (&fmt)[N], std::size_t n)
    {
        return
            ...
            fmt[n] != '%' ? Format<T, Ts...>::check(fmt, n + 1)

            // %%
            :
            isLiteralPercent(fmt, n) ? Format<T, Ts...>::check(fmt, n + 2)
...
#define PP_PARENTHESISE_WITH_TOKEN(r, token, i, e) BOOST_PP_COMMA_IF(i) token(e)

#define PP_CSV_SEQ_PARENTHESISE_WITH_TOKEN(...) \
    BOOST_PP_SEQ_FOR_EACH_I(                    \
        PP_PARENTHESISE_WITH_TOKEN, decltype, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))

#define PP_PERFORM_LOG_FORMAT_CHECK(fmt, ...)                                     \
    Format<BOOST_PP_IF(BOOST_PP_EQUAL(BOOST_PP_TUPLE_SIZE((, ##__VA_ARGS__)), 1), \
                       BOOST_PP_EXPAND,                                           \
                       PP_CSV_SEQ_PARENTHESISE_WITH_TOKEN)(__VA_ARGS__)>::check(fmt, 0)
