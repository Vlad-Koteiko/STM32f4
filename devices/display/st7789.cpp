...
{
    St7789::St7789(drivers::spi::SPI &spiScreen, drivers::port::GPIO &port, drivers::port::PIN_NUMBER dc,
                   drivers::port::PIN_NUMBER res, drivers::port::PIN_NUMBER bck, std::uint16_t width,
                   std::uint16_t height) :  spiScr(spiScreen), port(port), DC(dc), RES(res), BCK(bck){
        stWidth = width;
        stHeight = height;
    ...
    void St7789::MemAccessModeSet(std::uint8_t rotation, std::uint8_t vertMirror, std::uint8_t horizMirror,
        ...
        switch (rotation) {
            case 0:
                value = 0; break;
            case 1:
                value = ST7789_MADCTL_MX; break;
            case 2:
                value = ST7789_MADCTL_MY; break;
            case 3:
                value = ST7789_MADCTL_MX | ST7789_MADCTL_MY; break;
            case 4:
                value = ST7789_MADCTL_MV; break;
            case 5:
                value = ST7789_MADCTL_MV | ST7789_MADCTL_MX; break;
            case 6:
                value = ST7789_MADCTL_MV | ST7789_MADCTL_MY; break;
            case 7:
                value = ST7789_MADCTL_MV | ST7789_MADCTL_MX | ST7789_MADCTL_MY; break;
        }
    ...
    void St7789::FillRect(std::uint16_t x, std::uint16_t y, std::uint16_t w, std::uint16_t h, std::uint16_t color) noexcept {
        if ((x >= stWidth) || (y >= stHeight)) return;
        if ((x + w) > stWidth) w = stWidth - x;
        if ((y + h) > stHeight) h = stHeight - y;
        SetWindow(x, y, x + w - 1, y + h - 1);
        for (uint32_t i = 0; i < (h * w); i++) RamWrite(&color, 1);
    }

    void St7789::SetWindow(std::uint8_t x0, std::uint8_t y0, std::uint8_t x1, std::uint8_t y1) noexcept {
        ColumnSet(x0, x1);
        RowSet(y0, y1);
        SendCmd(RAMWR);
    }
    ...
    void St7789::DrawRectangle(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color) noexcept {
    ...
    void St7789::DrawRectangleFilled(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t fillcolor) noexcept {
        if (x1 > x2)
    ...
    void St7789::DrawLineSlow(int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color) noexcept {
    ...
    void St7789::DrawCircleFilled(int16_t x0, int16_t y0, int16_t radius, uint16_t fillcolor) noexcept {
        int x = 0;
        int y = radius;
    ...
    void St7789::PrintChar(int16_t x, int16_t y, uint16_t color, uint8_t character) noexcept {
        ...
        if((consolas_9ptFontInfo.start_character <= character) && (character <= consolas_9ptFontInfo.end_character))
        {
...
//            printf("Offset next %d", offsetNext);
    ...
    void St7789::PrintString(int16_t x, int16_t y, uint16_t color, uint8_t *string, uint16_t count) noexcept {
