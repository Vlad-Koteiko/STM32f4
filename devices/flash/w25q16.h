...
namespace devices::flash
{
    ...
    public:
        W25Q16(drivers::spi::SPI &flashSPI, drivers::port::GPIO &csPorT, drivers::port::PIN_NUMBER csPiN);
        ...
        void ReadBytes(std::uint32_t address, std::uint8_t *rxData, std::uint32_t bytesToRead) noexcept;
        void WriteBytes(std::uint32_t address, std::uint8_t *txData, std::uint32_t bytesToWrite) noexcept;
    };
}
